...
**Status: RESOLVED (2026-08-19 ruling; implementation LANDED as A21,
2026-08-19, `19e8498`).** **Owner: the Bearer task (no filter change on the
...
[`docs/done/2026-08-23-A22b-admin-factor-management-spec.md`](../done/2026-08-23-A22b-admin-factor-management-spec.md)):
...
- **A24 — force-enrol view (Ruling 4's "for now" companion). RESOLVED
  ...
  `docs/done/2026-08-25-A24-force-enrol-spec-delta.md` (all §9 rulings AS
  RECOMMENDED). **Closed 2026-08-26 as PR #28 (`9bd4ed9`):** seam-pinned
  (`A24ForceEnrolSeamTest`, 10 pins), one booted leg
  (`MfaAdminIT.setupPendingUserHasClearFactorsRecoveryPath`), headful-Chromium
  acceptance walk end-to-end (`ACCEPT_A24_OK`, `d99bbd9`). **Deviation
  OVERRIDE-ACCEPTED by mads (2026-08-28 — the work was needed sooner): no
  qwen re-walk; deviation closed.** §7's "walked ... on
  `qwen3.8:27b-mtp-q8_0`, stated exactly in the report" was NOT met (walk's
  terminal marker self-reports `ox-alpha model via OpenClaw subagent`); mads
  accepted that as an explicit override. **§7 booted IT matrix LANDED
  2026-08-28 (`f631503`, branch `a24-it-matrix`): `MfaAdminA24IT` — 5 legs /
  13 pins, all green; found and fixed ONE genuine product defect (D16:
  the forced-setup verify cleared the marker + granted trust in memory
  BEFORE the failing save and never rolled them back, so a
  `persistence_failed` user kept a working session the disk did not back;
  `MfaController.postVerify` now restores the pre-verify state before
  answering). Full `mvn -o -B clean verify` green (298 tests). The
  residual below is this entry's original scope, now RESOLVED. Original entry: when an admin enforces MFA fleet-wide the
...
| A22-b — no admin UI to manage *other users'* factors (lockout recovery dead end) | Option (A) per spec ruling 1: new `MfaAdminController` (mount `mfaAdmin`, NOT gate allow-listed) with enrolled-only roster + `clearFactors`/`revokeTrust`; two-axis guard (permission → credential) on a pure seam, self-management refused, typed-id confirm server-side, audit log on both mutations; gate carve-out (segment-precise bare-`/mfa` allow-list + `mfaadmin` in `isSecurityPath`); spec-internal "zero filter changes" contradiction flagged in the A22 entry. Plus the restart-survival leg (spec §7 case 3 + §10, 2026-08-23): `MfaAdminIT` leg 6 — clear → on-disk anchor → `rule.restart()` → victim reloaded-still-cleared + gate-passes + re-enrolls end to end + admin factors survive and stay live. Real-browser closure: context-rooted `mfa-admin.js` URL (red→green rendered-wire pin), both colour schemes, typed clear, victim password-only recovery + fresh TOTP, actual `hpi:run` restart, admin/victim factors re-proven, reader 403. | this commit (A22-b, 2026-08-23) | Spec-governed: see `docs/done/2026-08-23-A22b-admin-factor-management-spec.md` + the A22 entry's "Landed" note. Follow-up tracked as A24 (force-enrol view, Ruling 4's "for now"). |
| A24 — force-enrol view + `forceEnrol` verb + first-time `/mfa` setup (Ruling 4's "for now" companion) | Spec delta, all 16 rulings AS RECOMMENDED (2026-08-25): `forceEnrol` on the admin surface (typed-id confirm, email-only provisioning + `forcedSetupPending` marker, obligation-never-proof), REQUIRED-only not-enrolled complement, first-time `/mfa` setup variant (zero filter changes). Seam-pinned `A24ForceEnrolSeamTest` (10 pins); booted `MfaAdminIT` leg 7 (setup-pending recovery); headful-Chromium acceptance walk end-to-end (`ACCEPT_A24_OK`, e-mailed code leg via local SMTP sink). **Deviation OVERRIDE-ACCEPTED by mads 2026-08-28 (no qwen re-walk: the required qwen-model walk ran on a different harness, self-reported `ox-alpha via OpenClaw subagent`).** **§7 booted IT matrix landed 2026-08-28 (`f631503`): `MfaAdminA24IT` 5 legs green (journey+`rule.restart()`, guard pins incl. gate-bounce vs 403-seam split, closed setup variant + D5 idempotence/correction, wrong/expired/cooldown + D16 persistence arm, policy/identity edges); red-first found the D16 in-memory-rollback hole in `postVerify` — marker-clear and trust grant now roll back before `persistence_failed`, so the bounce stands (no session, no trust) and the disk obligation is intact. Regression: full `clean verify` 298/298 green.** | `d99bbd9` (PR #28) + `f631503` (IT matrix + D16) | Spec of record: `docs/done/2026-08-25-A24-force-enrol-spec-delta.md`. |
