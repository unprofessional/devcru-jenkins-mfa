...
- **A24 — force-enrol view (Ruling 4's "for now" companion). RESOLVED
  ...
  acceptance walk end-to-end (`ACCEPT_A24_OK`, `d99bbd9`). **Open deviation
  (awaiting mads's ruling):** §7's binding "walked ... on
  `qwen3.8:27b-mtp-q8_0`, stated exactly in the report" was NOT met — the
  walk's terminal marker self-reports `ox-alpha model via OpenClaw subagent`,
  and the full §7 booted IT matrix (journey-with-restart, guard pins,
  setup-does-not-bypass, policy edges) remains unimplemented. A qwen re-walk +
  remaining IT legs stand unless ruled otherwise. Original entry: when an admin enforces MFA fleet-wide the
...
| A24 — force-enrol view + `forceEnrol` verb + first-time `/mfa` setup (Ruling 4's "for now" companion) | Spec delta, all 16 rulings AS RECOMMENDED (2026-08-25): `forceEnrol` on the admin surface (typed-id confirm, email-only provisioning + `forcedSetupPending` marker, obligation-never-proof), REQUIRED-only not-enrolled complement, first-time `/mfa` setup variant (zero filter changes). Seam-pinned `A24ForceEnrolSeamTest` (10 pins); booted `MfaAdminIT` leg 7 (setup-pending recovery); headful-Chromium acceptance walk end-to-end (`ACCEPT_A24_OK`, e-mailed code leg via local SMTP sink). **Open deviation (awaiting mads): the required qwen-model walk ran on a different harness (self-reported `ox-alpha via OpenClaw subagent`), and the full §7 booted IT matrix is unimplemented.** | `d99bbd9` (PR #28, merged 2026-08-26 as `9bd4ed9`) | Spec of record: `docs/done/2026-08-25-A24-force-enrol-spec-delta.md`. |
